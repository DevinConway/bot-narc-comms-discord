...
module.exports.run = async (client, message, args) => {
	...
	if (message.member.hasPermission('ADMINISTRATOR')) {
		const commEmbed0 = new Discord.RichEmbed()
			.setAuthor(`Here are the commands available to you ${author0}!`)
			.setColor('RANDOM')
			.addField('__mod!help__', '_This command will send you a message of all commands_\n**Use:** m!help')
			.setThumbnail(message.author.displayAvatarURL)
			.setFooter('Sent')
			.setTimestamp([new Date().toUTCString()])
		message.channel.send(`You've got mail ${message.author.toString()}! 📬`);
		...
		const commEmbed1 = new Discord.RichEmbed()
			...
			.setColor('RANDOM')
			.addField('__mod!kick__', '_This command will kick the user that you stated_\n**Use:** m!kick [user] [reason]')
			.addField('__mod!ban__', '_This command will ban the user that that you stated_\n**Use:** m!ban [user] [reason]')
			//.addField('__mod!mute__', '_This command will mute the user that you stated_\n**Use:** m!mute [user]')
			//.addField('__mod!unmute__', '_This command will unmute the user that you stated_\n**Use:** m!unmute [user]')
			.addField('__mod!purge__', 'This command will delete the amount of messages you stated\n**Use:** m!purge [amount]')
			.setFooter('Sent')
			.setTimestamp([new Date().toUTCString()])
